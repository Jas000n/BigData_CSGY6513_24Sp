...
class MHSketch(InnerProdSketch):
    ...
    def inner_product(self, other: 'MHSketch', use_numba=False) -> float:
        if use_numba:
            ...
            union_size_est = (1 / mean_min - 1)
            sum_k = sum(
                [(va * vb) for ha, hb, va, vb in zip(self.sk_hashes, other.sk_hashes, self.sk_values, other.sk_values)
                 if ha == hb])
...
class WMHSketch(InnerProdSketch):
    def __init__(self, sk_hashes: np.ndarray, sk_values: np.ndarray, vecotr_l2: float, sketch_size: int,
                 p: float) -> None:
        self.sk_hashes: np.ndarray = sk_hashes
        self.sk_values: np.ndarray = sk_values
    ...
    def inner_product(self, other: 'WMHSketch', use_numba=False) -> float:
        if use_numba:
            return self.inner_product_numba(self.sk_hashes, self.sk_values, other.sk_hashes, other.sk_values,
                                            self.vector_l2, other.vector_l2, self.sketch_size, self.p)
        else:
            mean_min = np.mean([min(hA, hB) for hA, hB in zip(self.sk_hashes, other.sk_hashes)])
...
class WMH(InnerProdSketcher):
    ...
    def sketch_geometric_numba(vec_norm_nonzeroIndex, vec_norm2_repeat, sample_size, seed):
        ...
        return vec_hash
class KMV_sketch(algorithm_adaptor):
    def __init__(self, sketch_size):
        super().__init__("KMV_sketch")
        self.sketch_size = sketch_size

    def sketch(self, vector, sketch_size):
        sketcher = WMH(sketch_size=sketch_size,seed=28321931,p=0.5)
        return sketcher.sketch(vector)
class MH_sketch(algorithm_adaptor):
    def __init__(self, sketch_size):
        super().__init__("MH_sketch")
        self.sketch_size = sketch_size

    def sketch(self, vector, sketch_size):
        sketcher = WMH(sketch_size=sketch_size,seed=28321931)
        return sketcher.sketch(vector)