import numpy as np
from algorithms.abstract_class import InnerProdSketcher, InnerProdSketch, compute_adaptive_threshold, hash_kwise
...
class TSSketch(InnerProdSketch):
    def __init__(self, sk_indices: np.ndarray, sk_values: np.ndarray, threshold: float, vector_norm: float,
                 norm: int) -> None:
    ...
    @staticmethod
    @njit(parallel=False)
    def inner_product_numba(sk_indicesA, sk_valuesA, normA, thresholdA, vector_normA, sk_indicesB, sk_valuesB, normB,
                            thresholdB, vector_normB):
        i = 0
        j = 0
        ip_est = 0
        cnt = 0
    ...
    def inner_product(self, other: 'TSSketch', use_numba=False) -> float:
        if use_numba:
            return self.inner_product_numba(self.sk_indices, self.sk_values, self.norm, self.threshold,
                                            self.vector_norm, other.sk_indices, other.sk_values, other.norm,
                                            other.threshold, other.vector_norm)
        else:
            ip_est = 0
            cnt = 0
...
class threshold_sampling(algorithm_adaptor):
    def __init__(self,sketch_size):
        super().__init__("Threshold Sampling")
        self.sketch_size = sketch_size
    def sketch(self, vector, sketch_size):

        sketcher = TS(sketch_size=sketch_size, seed=28321931,norm=2)
...
#     sketcher = threshold_sampling(100)
#     sketched_A = sketcher.sketch(vectorA,100)
#     sketched_B = sketcher.sketch(vectorB,100)
#     print("Estimated Inner Product:", sketched_A.inner_product(sketched_B))
